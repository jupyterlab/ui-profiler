import { IScenario, IResult, IOutcome, benchmark } from './benchmark';
...
interface IOptions {
  /**
   * Number of repeats for the benchmark, default 3.
   */
  n?: number;
}

export async function benchmarkStyleSheets(
  scenario: IScenario,
  options: IOptions = {}
...
interface IStyleOptions extends IOptions {
  skipPattern?: string;
  // TODO skip entire style sheets
}

export async function benchmarkStyleRules(
  scenario: IScenario,
...
interface IRuleGroupOptions extends IStyleOptions {
  /**
   * Block size to start with.
   */
  minBlocks?: number;
  /**
   * Maximal block size.
   */
  maxBlocks?: number;
  sheetRandomizations?: number;
}

export async function benchmarkRuleGroups(
  scenario: IScenario,
  options: IRuleGroupOptions = {}
